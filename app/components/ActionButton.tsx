...
const ActionButton = (props: ActionButtonProps) => {
  ...
  const handleClick = () => {
    ...
    if (commentIdToReply) {
      action === "reply"
        ? onReplyComment
          ? onReplyComment(newReply, commentIdToReply)
          : null
        : null;
    ...
    if (replyIdToReply) {
      action === "reply"
        ? onReplyReply
          ? onReplyReply(newReply, replyIdToReply)
          : null
        : null;
      onIsReplyingChange ? onIsReplyingChange() : null;
    }
    if (commentIdToUpdate) {
      action === "update"
        ? onUpdateComment && updatedCommentContent
          ? onUpdateComment(updatedCommentContent, commentIdToUpdate)
          : null
        : null;
    ...
    if (replyIdToUpdate) {
      action === "update"
        ? onUpdateReply && updatedReplyContent
          ? onUpdateReply(updatedReplyContent, replyIdToUpdate)
          : null
        : null;
      onIsEditingChange ? onIsEditingChange() : null;
    }
